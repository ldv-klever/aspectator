          ...
          while (1)
            {
              if (!arg_values[j])
                {
                  /* TODO. What happens here? LDV_CPP_FATAL_ERROR ("Can't get the following token"); */
                  break;
                }
...
{
  const char *file_path_aspect = NULL;
  ...
  if (LDV_STR_END_EQ (file_path_aspect, file_path_source))
    {
