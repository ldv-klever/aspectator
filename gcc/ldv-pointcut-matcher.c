...
static bool ldv_ismodified_first = false, ldv_ismodified_second = false;
...
ldv_match_expr_2 (tree t)
{
  ...
  switch (code)
    {
    case ADDR_EXPR:
      /* We consider just addressed functions. So it fixes strange bug. */
      if (TREE_CODE (LDV_OP1) == FUNCTION_DECL)
        {
          ...
          if (ldv_func_called_matched)
            {
              LDV_OP1 = ldv_func_called_matched;
              ldv_func_called_matched = NULL;
            }
        }
...
if (0)
      for (chain = LDV_OP2, arg_numb = 1
        ...
        ; chain = TREE_CHAIN (chain), arg_numb++)
        {
          ...
          /* A value contains information on a function arguments expression. */
          value = TREE_VALUE (chain);

          if (TREE_CODE (value) == ADDR_EXPR)
            {
              op1 = TREE_OPERAND (value, 0);

              value_code = TREE_CODE (op1);

              /* Function, variable and field declarations that have an array
                 type are interest for us at the moment. */
              if (value_code == FUNCTION_DECL && DECL_NAME (op1))
                {
                  func_arg_info_new->func_arg_info_kind = LDV_FUNC_ARG_INFO_FUNC_NAME;

                  /* Store a function name used as a value of a function
                     argument. */
                  func_arg_info_new->func_name = IDENTIFIER_POINTER (DECL_NAME (op1));
                }
              else if (value_code == VAR_DECL && (array_size_tree = LDV_ARRAY_SIZE (op1)))
                {
              ...
              else if ((array_size = ldv_array_field_size (value)))
                {
                  func_arg_info_new->func_arg_info_kind = LDV_FUNC_ARG_INFO_ONE_DIM_ARRAY_SIZE;
          ...
          else if (TREE_CODE (value) == VAR_DECL)
            {
              ...
              for (var_array_list = ldv_var_array_list
                ...
                ; var_array_list = ldv_list_get_next (var_array_list))
                {
                  ...
                  /* Compare unique ids. */
                  if (var_array->uid == DECL_UID (value)
    ...
    case FUNCTION_DECL:
      /* Try to match a function declaration for a call join point. */
      ldv_match_func (t, LDV_PP_CALL);

      /* Obtain a function close brace location. A function structure is
         available for a function that has a definition. For a function that has
         a declaration a place of its finishing ';' is used. */
      if ((cfunc = DECL_STRUCT_FUNCTION (t)) != NULL)
        func_close_brace_location = expand_location (cfunc->function_end_locus);
      else
        func_close_brace_location = expand_location (DECL_SOURCE_LOCATION (t));
    ...
    case MODIFY_EXPR:
      ...
      if (ldv_func_called_matched)
        ...
        {
          /* See just on a parameter or a variable declaration that is
             modified. */
          if (TREE_CODE (LDV_OP2) == PARM_DECL || TREE_CODE (LDV_OP2) == VAR_DECL)
            ldv_ismodified_second = true;

          ldv_match_expr (LDV_OP2);
          ldv_ismodified_second = false;

          /* Change a modifier to an aspect function call if get was matched. */
          if (ldv_func_called_matched)
            {
              /* Create a list of arguments of a called in future function. */
              list = build_tree_list (NULL_TREE, LDV_OP2);
    ...
    case STATEMENT_LIST:
      /* Match parts of statement. */
      for (si = tsi_start (t); !tsi_end_p (si); tsi_next (&si))
        ldv_match_expr (tsi_stmt (si));

      break;

    /* A parameter declaration is processed in the same way as a local variable.
       It assumes that all parameters have names and their contextes is a
    ...
    case VAR_DECL:
      ...
      if (DECL_NAME (t) && (TREE_CODE (DECL_NAME (t)) == IDENTIFIER_NODE))
        {
          /* Try to match a variable declaration for set and get joinpoints
             correspondingly. */
          if (ldv_ismodified_first)
            {
              if (isvar_global)
                ldv_match_var (t, LDV_PP_SET_GLOBAL);
              else
                ldv_match_var (t, LDV_PP_SET_LOCAL);
            }
          else if (ldv_ismodified_second)
            {
              if (isvar_global)
                ldv_match_var (t, LDV_PP_GET_GLOBAL);
              else
                ldv_match_var (t, LDV_PP_GET_LOCAL);
            }
          ...
          if (isvar_global)
            var_func_location = expand_location (DECL_SOURCE_LOCATION (t));
          else
            var_func_location = expand_location (DECL_SOURCE_LOCATION (DECL_CONTEXT (t)));
